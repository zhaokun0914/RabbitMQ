...
public class ProducerController {
    ...
    public void sendMessage(@PathVariable String message) {
        ...
        log.info("==> 给routing key为：{} 的队列发送消息：{}，消息的ID为：{}", ConfirmConfig.CONFIRM_QUEUE_KEY2, message, correlationData2.getId());
        rabbitTemplate.convertAndSend(ConfirmConfig.CONFIRM_EXCHANGE_NAME, ConfirmConfig.CONFIRM_QUEUE_KEY2, message, correlationData2);

        // 恢复回调函数
        // rabbitTemplate.setConfirmCallback(null);
        // rabbitTemplate.setReturnsCallback(null);
        // rabbitTemplate.setMandatory(false);
    }

    private void setCallback() {
        // 该回调函数会在交换机接收到消息的时候调用，一个 RabbitTemplate 只能设置一个 ConfirmCallback 回调函数
        // spring.rabbitmq.publisher-confirm-type=correlated 给这个属性设置的回调函数
        ...
        // 设置[回退消息]交给[谁处理]
        // spring.rabbitmq.publisher-returns=true 给这个属性设置的回调函数
